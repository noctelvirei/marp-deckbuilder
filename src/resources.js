...
export const imageExtensions = ['.svg', '.png', '.jpg', '.jpeg', '.webp', '.gif']

export function normalizeResourceReference(value, options = {}) {
  ...
  const resourcePath =
    defaultFolder && !normalized.includes('/') ? `${defaultFolder}/${normalized}` : normalized
  return `resource:${resourcePath}`
}

export function resolveResourceFile(value, resourcesDir = 'resources') {
  ...
  const resourcePath = rawValue.startsWith('resource:')
    ? rawValue.slice('resource:'.length)
    : rawValue
