...
// src/report/content.js
var markdown = new lib_default({
  html: true,
  linkify: true,
  typographer: true
});
function prepareReportContent(source, options = {}) {
...
function reportRichType(rich) {
  ...
  return attrs.find((attr) => rich.attr(`data-deck-rich-${attr}`) !== void 0) || "rich";
}
...
function decorateReportContent(content) {
  ...
  root("h1,h2").each((_, element) => {
    const heading2 = root(element);
    const label = heading2.text().replace(/\s+/g, " ").trim();
    if (!label) return;
    const id = uniqueSlug(heading2.attr("id") || label, used);
    heading2.attr("id", id);
    toc.push({ id, label });
  });
  return {
...
// src/report/document.js
function reportLogo(brand = {}, surface = "light") {
...
function reportCustomerLogo(frontmatter = {}) {
...
<body>
  <main class="deck-report report-${escapeHtmlAttr2(surface)}">
    <header class="report-cover">
      ${customerLogo ? `<img class="report-customer-logo" src="${escapeHtmlAttr2(customerLogo)}" alt="${escapeHtmlAttr2(customerName)} logo">` : ""}
      ${logo ? `<img class="report-logo" src="${escapeHtmlAttr2(logo)}" alt="${escapeHtmlAttr2(brandName)} logo">` : ""}
...
function renderReportToc(toc = []) {
  const links = toc.map((item) => `<a href="#${escapeHtmlAttr2(item.id)}">${escapeHtml3(item.label)}</a>`).join("\n");
