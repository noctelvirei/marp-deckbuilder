...
- Follow the report separator convention: semicolons separate repeated records or items, pipes separate fields inside one record, and commas separate simple one-dimensional chart series. Use `items="Scope: Pilot; Platform: v2"` for `report-key-values`, never `items="Scope: Pilot|Platform: v2"`.
